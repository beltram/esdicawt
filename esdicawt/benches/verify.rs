...
use std::{fs::File, hint::black_box, os::raw::c_int, path::Path};
...
fn verifier<H: digest::Digest + Clone>(i: &usize) -> (Ed25519Verifier<VarSizePayload>, bytes::Bytes, VerifierParams<'_>, CoseKeySet) {
...
fn shallow_verifier<H: digest::Digest + Clone>(i: &usize) -> (Ed25519Verifier<VarSizePayload>, bytes::Bytes, ShallowVerifierParams, CoseKeySet) {
    let (holder, holder_params, sd_cwt, cks) = holder::<H>(i);
...
criterion::criterion_group! {
    ...
    targets = issue_bench, holder_bench, verifier_bench, shallow_verifier_bench
}
