...
pub trait Verifier {
    ...
    fn digest(&self, sd_alg: SelectiveDisclosureHashAlg, data: &[u8]) -> Result<Vec<u8>, Self::Error>;
    ...
    > {
        let kbt_tagged = KeyBindingTokenTagged::<
            Self::IssuerProtectedClaims,
            Self::IssuerUnprotectedClaims,
            Self::IssuerPayloadClaims,
            Self::KbtProtectedClaims,
    ...
    > {
        ...
        let (sd_cwt, sd_cwt_bytes) = kbt_protected.issuer_sd_cwt.to_pair_mut()?;
        let key_confirmation = &sd_cwt.0.payload.to_value()?.key_confirmation;
...
mod tests {
    ...
    fn verify<D: CustomClaims>(disclosable_claims: D) -> KeyBindingTokenVerified<NoClaims, NoClaims, D, NoClaims, NoClaims, NoClaims, D> {
    ...
    fn generate<D: CustomClaims>(
        ...
        KeyBindingTokenTagged<NoClaims, NoClaims, D, NoClaims, NoClaims, NoClaims, D>,
    ) {
...
pub mod test_utils {
    ...
    impl<DisclosedClaims: CustomClaims> Verifier for HybridVerifier<DisclosedClaims> {
        ...
        fn digest(&self, sd_alg: SelectiveDisclosureHashAlg, data: &[u8]) -> Result<Vec<u8>, Self::Error> {
            ...
            Ok(match sd_alg {
                SelectiveDisclosureHashAlg::Sha256 => sha2::Sha256::digest(data).to_vec(),
                _ => unreachable!(),
            })
        }
    }
